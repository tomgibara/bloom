...
public class BloomMapImplTest extends TestCase {
	...
	static final Storage<Integer> storage = StoreType.of(int.class).settingNullToDefault().storage();
	...
	public void testBasic() {
		...
		BloomMap<Integer, Integer> ca = Bloom.withHasher(hasher, 10).newMap(storage, lattice);
	...
	public void testKeys() {
		...
		hasher = hasher.ints().sized(DEFAULT_SIZE);
		OrderedLattice<Integer> lattice = new OrderedLattice<>(10000, 0);
		BloomMap<Integer, Integer> map = Bloom.withHasher(hasher, 10).newMap(storage, lattice);
